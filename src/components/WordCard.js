...
const WordCard = ({ wordObj }) => {
  ...
  // ✅ Kelime her değiştiğinde tüm çeviri state'lerini sıfırla
  useEffect(() => {
      ...
      <div className="space-y-4 text-left">
              ...
              {def?.meaning && (
                <span
                  className={`font-bold text-lg ${
                    idx === 0 ? "text-indigo-900" : "text-slate-700"
                  }`}
                >
                  {def.meaning}
                </span>
                ...
                <div className="flex items-start justify-between gap-2">
                  <p
                    className={`text-sm italic font-medium ${
                      idx === 0 ? "text-indigo-500" : "text-slate-500"
                    }`}
                  >
                    "{def.engExplanation}"
                  </p>
                  <div className="flex gap-1">
                    ...
                    <button
                      onClick={(e) =>
                        handleTranslateDef(idx, def.engExplanation, e)
                      }
                      className="opacity-50 hover:opacity-100 p-1 bg-white rounded-full shadow-sm"
                    >
                      {loadingDefs[idx] ? (
        ...
        {(wordObj.advLy || wordObj.compEr || wordObj.superEst) && (
          <div className="bg-orange-50 p-3 rounded-xl border border-orange-100 text-left space-y-1.5 mt-2">
            <div className="text-[10px] uppercase tracking-wide text-orange-400 font-bold mb-1">
              Sıfat & Zarf Halleri
            </div>
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="p-1.5 bg-white text-indigo-500 rounded-full border border-slate-200 hover:bg-indigo-50 transition-colors"
                >
                  {loadingSentence ? (
        ...
        {wordObj.tags &&
          Array.isArray(wordObj.tags) &&
          wordObj.tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mt-4 justify-center">
              {wordObj.tags.map((tag, i) => (
                <span
                  key={i}
                  className="text-[10px] font-bold text-slate-500 bg-slate-100 px-2 py-1 rounded-full flex items-center gap-1 border border-slate-200"
                >
