...
export default function AddWord() {
  ...
  const initialData = editingWord
    ? {
        ...editingWord,
        tags: Array.isArray(editingWord.tags) ? editingWord.tags : [],
  ...
  const addTag = (e) => { e.preventDefault(); if(tagInput.trim() && !formData.tags.includes(tagInput.trim())) { setFormData(prev => ({ ...prev, tags: [...prev.tags, tagInput.trim()] })); setTagInput(""); } };
      ...
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-6 my-8 overflow-y-auto max-h-screen">
        ...
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-slate-800">
            {isEditMode ? "Kelimeyi Düzenle" : (isAdmin ? "Sisteme Kelime Ekle" : "Yeni Kelime Ekle")}
          </h2>
