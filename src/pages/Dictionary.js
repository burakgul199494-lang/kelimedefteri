...
export default function Dictionary() {
  ...
  useEffect(() => {
    ...
    const filtered = allWords.filter(item => {
        ...
        // 3. Fiil ve İsim Çekimleri (EKSİKLER EKLENDİ)
        if (item.v2?.toLowerCase().includes(cleanTerm)) return true;          // sat
        if (item.v3?.toLowerCase().includes(cleanTerm)) return true;          // sat (participle)
        if (item.vIng?.toLowerCase().includes(cleanTerm)) return true;        // sitting
        if (item.plural?.toLowerCase().includes(cleanTerm)) return true;      // computers
        if (item.thirdPerson?.toLowerCase().includes(cleanTerm)) return true; // sits
    ...
    <div className="min-h-screen bg-slate-50 p-4">
      <div className="max-w-md mx-auto">
        ...
        <div className="relative group mb-6">
          ...
          <input
            type="text"
            placeholder="Kelime, anlam veya çekim ara..."
            value={term}
          ...
          />
          {term && (
            <button type="button" onClick={handleClear} className="absolute right-3 top-3 p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-100 rounded-full transition-colors">
              <X className="w-5 h-5" />
            </button>
          )}
        </div>
                ...
                <>
                    ...
                    {visibleCount < results.length && (
                        <button onClick={handleLoadMore} className="w-full py-4 bg-indigo-50 text-indigo-600 font-bold rounded-xl flex items-center justify-center gap-2 hover:bg-indigo-100 transition-colors">
                            <ArrowDownCircle className="w-5 h-5"/> Daha Fazla Göster ({results.length - visibleCount} kaldı)
                        </button>
                    )}
                </>
            )}
