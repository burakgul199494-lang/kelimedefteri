...
export default function Game() {
  ...
  const startSession = () => {
    ...
    if (playableWords.length === 0) {
      setSessionComplete(true); setSessionWords([]); 
    } else {
      const shuffled = [...playableWords].sort(() => 0.5 - Math.random());
      setSessionWords(shuffled.slice(0, 20));
      setCurrentIndex(0); setSessionComplete(false); setStats({ learned: 0, review: 0 });
    }
  };
  ...
  const handleSwipe = async (dir) => {
    ...
    setTimeout(async () => {
      ...
      if (currentIndex + 1 < sessionWords.length) {
        ...
        setSessionComplete(true); setSwipeDirection(null);
      }
    }, 300);
  };
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full">
           ...
           <h2 className="text-2xl font-bold text-slate-800 mb-2">{availableToPlay === 0 ? "Tüm Kelimeler Tamam!" : "Oturum Tamamlandı"}</h2>
           ...
           <div className="space-y-3 mb-6">
                ...
                <div className="flex justify-between text-sm text-slate-600 bg-slate-50 p-3 rounded-lg"><span>Çalışılabilir:</span><span className="font-bold text-green-600">{availableToPlay}</span></div>
           </div>

           <button onClick={startSession} disabled={availableToPlay === 0} className="w-full bg-blue-600 disabled:bg-slate-300 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 mb-3"><RotateCcw className="w-5 h-5"/> {availableToPlay > 0 ? "Yeni Oturum" : "Kelimeler Tükendi"}</button>
    ...
    <div className="flex flex-col min-h-screen bg-slate-100 overflow-hidden">
      <div className="bg-white shadow-sm p-4 z-10">
        <div className="max-w-md mx-auto">
          <div className="flex justify-between items-center mb-2"><button onClick={() => navigate("/")} className="text-slate-400 hover:text-slate-700"><X className="w-6 h-6"/></button><span className="text-xs font-semibold bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full">{currentIndex + 1} / {sessionWords.length}</span></div>
