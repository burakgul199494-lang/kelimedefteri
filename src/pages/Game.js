...
const WordCard = ({ wordObj }) => {
  ...
  // Her tanım için ayrı çeviri durumu
  ...
  // Kelime değiştiğinde (wordObj.word), eski çevirileri ve resmi temizle, yenisini yükle.
  useEffect(() => {
    // 1. State Temizliği
    ...
    <div className="relative w-full max-w-sm bg-white rounded-3xl shadow-xl overflow-hidden border border-slate-100 mb-4 mx-auto">
      ...
      <div className="w-full h-48 bg-slate-100 relative flex items-center justify-center overflow-hidden">
        ...
        {!loadingImage && !imageUrl && (
            <ImageIcon className="w-12 h-12 text-slate-300 opacity-50"/>
        )}
        ...
        {!loadingImage && imageUrl && (
            ...
            />
            <a 
                href={imageUrl.photographerUrl} 
                target="_blank" 
                rel="noopener noreferrer" 
                className="absolute bottom-1 right-1 text-[8px] text-white bg-black/50 px-1 rounded opacity-70 hover:opacity-100"
            >
            ...
            <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 mt-2">
                ...
                {sentenceTranslation && (
                    <div className="mt-2 pt-2 border-t border-slate-200 animate-in fade-in">
                        <p className="text-slate-800 text-sm font-medium">TR: {sentenceTranslation}</p>
                    </div>
                )}
            </div>
