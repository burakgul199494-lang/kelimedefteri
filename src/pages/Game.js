...
import { X, RotateCcw, Home, Target, Check, Trophy, BookOpen } from "lucide-react";
...
export default function Game() {
  ...
  const startSession = () => {
    ...
    const unknown = all.filter(w => !knownWordIds.includes(w.id));
    
    if (unknown.length === 0) {
      setSessionComplete(true);
      setSessionWords([]); // Boş liste
    } else {
      const shuffled = [...unknown].sort(() => 0.5 - Math.random());
      setSessionWords(shuffled.slice(0, 20)); // Her oturum 20 kelime
      setCurrentIndex(0);
  ...
  if (sessionComplete) {
    const all = getAllWords();
    const remaining = all.length - knownWordIds.length;
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50 p-6 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md w-full">
           {remaining === 0 ? <Trophy className="w-20 h-20 text-yellow-500 mx-auto mb-4"/> : <BookOpen className="w-16 h-16 text-blue-500 mx-auto mb-4"/>}
           <h2 className="text-2xl font-bold text-slate-800 mb-2">{remaining === 0 ? "Tebrikler!" : "Oturum Tamamlandı"}</h2>
           {remaining === 0 ? <p className="text-slate-600 mb-6">Tüm kelimeleri öğrendin!</p> : (
             <>
               ...
               <p className="text-slate-600 mb-6">Kalan kelime: {remaining}</p>
             </>
           )}
           <button onClick={startSession} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-6 rounded-xl flex items-center justify-center gap-2 mb-3"><RotateCcw className="w-5 h-5"/> Yeni Oturum</button>
