...
// iOS tarzı kırpma için kütüphane
import Cropper from "react-easy-crop";

// --- Helper Fonksiyonlar (iOS tarzı kırpma motoru) ---
...
export default function SentenceAnalysis() {
  ...
  // Normalize: teacher's → teacher, walking, → walking
  lower = lower.replace(/'s$/, "");      // sondaki 's kaldır
  lower = lower.replace(/[^a-z]/g, "");  // noktalama temizle

  // Dynamic system word içinde var mı?
  if (dynamicSystemWords.some(sw => sw.word.toLowerCase().trim() === lower)) {
    return true;
  }

  // Custom user words içinde var mı?
  if (
    customWords.some(
      cw =>
  ...
  return (
    ...
    {/* === QUICK ADD MODAL BURAYA GELECEK === */}
    {quickAddWord !== null && (
      <QuickAddModal
        word={quickAddWord}
        ...
        {analysisResult && (
          <div className="space-y-4">
            ...
            {analysisResult.correction?.hasError && (
              <div className="bg-red-50 p-5 rounded-2xl border border-red-100">
                <h3 className="text-xs font-bold text-red-500 uppercase mb-3 flex items-center gap-2">
                  <AlertTriangle className="w-4 h-4" /> Gramer Hatası
                </h3>
            ...
            <div className="bg-indigo-50 p-5 rounded-2xl border border-indigo-100">
              ...
              <p className="text-lg text-slate-800">{analysisResult.turkishTranslation}</p>
            </div>
            ...
            <div className="bg-teal-50 p-5 rounded-2xl border border-teal-100">
              <div className="flex items-center justify-between mb-3 border-b border-teal-100 pb-2">
                <h3 className="text-xs font-bold text-teal-600 uppercase flex items-center gap-2">
                  <Brain className="w-4 h-4" /> Analiz Özeti
                </h3>
