...
import { ArrowLeft, Mic, Square, Volume2, RefreshCw, Trophy, AlertCircle, StopCircle } from "lucide-react"; // StopCircle eklendi
...
export default function Pronunciation() {
  ...
  // Okuma durumu (Konuşuyor mu?)
  const [isSpeaking, setIsSpeaking] = useState(false);

  const recognitionRef = useRef(null);

  // --- YENİ: SAYFADAN ÇIKINCA SESİ KES ---
  useEffect(() => {
    // Temizlik Fonksiyonu (Cleanup)
    return () => {
        window.speechSynthesis.cancel(); // Sayfadan çıkarken SUS.
    };
  }, []);
  ...
  const handleSpeak = () => {
    ...
    // Önce varsa eski konuşmayı durdur
    ...
    // Konuşma başladığında ve bittiğinde state güncelle
  ...
  const toggleMic = () => {
    if (!text) { alert("Lütfen cümle yaz."); return; }
    
    // Mikrofon açılırsa okumayı durdur
    window.speechSynthesis.cancel(); 
    setIsSpeaking(false);
    ...
    if (isListening) {
        ...
        setSpokenText(""); setScore(null); setFeedback(null);
  ...
  const calculateScore = (target, originalSpoken) => {
      ...
      if (cleanTarget.length > 0) calculatedScore = Math.round((matchCount / cleanTarget.length) * 100);
      if (calculatedScore > 100) calculatedScore = 100;

      setScore(calculatedScore);

      if (calculatedScore === 100) setFeedback({ type: "success", msg: "Mükemmel! 🎉" });
      else if (calculatedScore >= 70) setFeedback({ type: "success", msg: "Gayet İyi! 👍" });
      else if (calculatedScore >= 40) setFeedback({ type: "warning", msg: "Fena değil. 🤔" });
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 space-y-4">
            ...
            <div className="flex gap-2">
                {isSpeaking ? (
                    ...
                    >
                        <StopCircle className="w-5 h-5"/> Durdur
                    </button>
                ) : (
                    <button 
                        onClick={handleSpeak} 
