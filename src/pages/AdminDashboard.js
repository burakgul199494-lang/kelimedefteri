...
export default function AdminDashboard() {
  ...
  const handleTranslate = async (e) => {
      ...
      if (res.success) setTranslatedEnWord(res.word);
      else alert("Çeviri hatası: " + res.message);
  ...
  const handleMagicAdd = async (e) => {
      ...
      if (aiResult.success && aiResult.data) {
          ...
          for (const wordObj of wordsArray) { await handleSaveSystemWord(wordObj); }
          setMagicStatus('success'); setMagicWord(""); setTimeout(() => setMagicStatus(null), 3000);
      } else { alert("AI Hatası: " + aiResult.message); }
      ...
      <div className={containerClass}>
          <div className="bg-slate-50 px-3 py-3 sm:px-4 border-b border-slate-100 flex justify-between items-start gap-2">
              <div className="flex flex-wrap items-center gap-2 flex-1 min-w-0">
                  ...
                  {isEdited && <span className="text-[9px] font-bold bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded border border-emerald-200 uppercase tracking-widest">Oxford (Düzenlendi)</span>}
                  ...
                  {!isOxford && <span className="text-[9px] font-bold bg-slate-100 text-slate-500 px-2 py-0.5 rounded border border-slate-200 uppercase tracking-widest">Manuel Ekleme</span>}
                  ...
                  {item.phonetic && <span className="text-xs text-indigo-400 font-serif italic bg-indigo-50 px-2 py-0.5 rounded-lg border border-indigo-100 shrink-0">/{item.phonetic.replace(/\//g, '')}/</span>}
                  <button onClick={(e)=>speak(item.word, e)} className="p-1.5 text-indigo-500 hover:bg-indigo-100 rounded-full transition-colors shrink-0"><Volume2 className="w-4 h-4"/></button>
              </div>
              <div className="flex gap-1.5 shrink-0">
                  {isBlacklisted ? (
                      <button onClick={() => removeFromBlacklist(item.id)} className="flex items-center gap-1 p-2 text-xs font-bold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-xl border border-emerald-200 transition-colors"><RotateCcw className="w-3.5 h-3.5"/> Geri Getir</button>
                  ) : (
                      <>
      ...
      <div className="max-w-md mx-auto w-full">
         ...
         <div className="bg-gradient-to-br from-indigo-600 to-purple-700 p-0.5 rounded-3xl mb-6 shadow-xl shadow-indigo-100 w-full">
            <div className="bg-white rounded-[22px] p-4 sm:p-5 w-full">
                <div className="flex items-center gap-2 mb-3 sm:mb-4 font-black text-slate-700 uppercase tracking-tighter text-sm sm:text-base"><Wand2 className="w-4 h-4 sm:w-5 sm:h-5 text-indigo-500 animate-pulse" /> Sihirli Ekleme (AI)</div>
                ...
                <div className="mt-4 pt-4 border-t border-indigo-100/30 w-full">
                    ...
                    <form onSubmit={handleTranslate} className="flex flex-col sm:flex-row gap-2 w-full">
                        ...
                        <button type="submit" disabled={isTranslating || !trWord.trim()} className="w-full sm:w-auto bg-slate-100 hover:bg-slate-200 text-slate-600 p-3 sm:px-5 rounded-xl font-bold transition-all disabled:opacity-50 text-sm flex items-center justify-center shrink-0">{isTranslating ? <Loader2 className="w-4 h-4 animate-spin" /> : "ÇEVİR"}</button>
                    </form>
                    {translatedEnWord && (
                        <div className="mt-3 flex flex-col sm:flex-row sm:items-center justify-between bg-emerald-50 p-3 rounded-xl border border-emerald-100 animate-in fade-in zoom-in-95 gap-3 w-full">
                            <div className="text-xs font-medium text-emerald-800 text-center sm:text-left">İngilizcesi: <span className="text-base sm:text-lg font-black ml-1 break-words">{translatedEnWord}</span></div>
                            <button onClick={() => { setMagicWord(translatedEnWord); setTranslatedEnWord(""); setTrWord(""); }} className="w-full sm:w-auto bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2.5 sm:py-2 rounded-lg text-xs font-bold shadow-sm shrink-0">Sihirli Ekle</button>
                        </div>
                    )}
                </div>
            </div>
         ...
         <div className="relative mb-6 w-full">
             ...
             <input type="text" placeholder="Kelime veya anlam ara..." value={search} onChange={e=>setSearch(e.target.value)} className="w-full pl-10 sm:pl-12 p-3 sm:p-4 bg-white border-2 border-slate-100 rounded-xl sm:rounded-2xl outline-none shadow-sm focus:border-indigo-200 transition-all font-medium text-slate-600 text-sm sm:text-base"/>
         </div>
             
         <div className="min-h-[500px] pb-20 w-full">
             <Virtuoso useWindowScroll data={filtered} itemContent={renderWordCard} />
         </div>
      </div>
    </div>
  );
