      ...
      <div className="w-full max-w-md space-y-6 mt-2">
        ...
        <div className="space-y-3 pb-8">
          ...
          <div className="grid grid-cols-2 gap-3">
              ...
              <button onClick={() => navigate("/game/sentence-builder")} className="bg-teal-600 text-white font-bold py-4 px-4 rounded-xl shadow-md flex flex-col items-center gap-2 active:scale-95"><Layout className="w-6 h-6"/><span className="text-sm">Cümle</span></button>
          ...
          {isAdmin && <button onClick={() => navigate("/admin")} className="w-full bg-slate-800 text-white font-bold py-3 px-6 rounded-xl shadow-md flex items-center gap-3"><Shield className="w-5 h-5 text-yellow-400"/> Admin Paneli</button>}
        </div>
      </div>
    </div>
  );
