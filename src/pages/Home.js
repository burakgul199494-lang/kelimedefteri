...
export default function Home() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      progress: all.length > 0 ? (totalLearned / all.length) * 100 : 0
    };
      ...
      <div className="w-full max-w-md space-y-6 mt-2">
        ...
        <div className="flex justify-between items-center w-full px-1">
           <div className="flex gap-2 w-full justify-between">
             ...
             <div className="flex gap-2">
                <button onClick={() => setShowCalendar(true)} className="p-2.5 bg-white rounded-xl shadow-sm border border-slate-200 text-slate-600 hover:text-indigo-600 active:scale-95 transition-transform"><Calendar size={18} /></button>
                <button onClick={() => setShowDailyQuests(true)} className="p-2.5 bg-white rounded-xl shadow-sm border border-slate-200 text-slate-600 hover:text-red-500 active:scale-95 transition-transform"><Target size={18} /></button>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
           ...
           <div className="flex justify-between text-sm divide-x divide-slate-100">
             <div onClick={() => navigate("/list/known")} className="text-center flex-1 px-1 cursor-pointer hover:bg-slate-50 rounded transition-colors group">
                <div className="font-bold text-slate-800 group-hover:text-green-600 text-lg flex items-center justify-center gap-1">{stats.review} <RotateCcw size={12} className="text-green-500"/></div>
             ...
             <div onClick={() => navigate("/list/waiting")} className="text-center flex-1 px-1 cursor-pointer hover:bg-slate-50 rounded transition-colors group">
                <div className="font-bold text-slate-800 group-hover:text-amber-500 text-lg flex items-center justify-center gap-1">{stats.waiting} <Hourglass size={12} className="text-amber-400"/></div>
             ...
             <div onClick={() => navigate("/list/unknown")} className="text-center flex-1 px-1 cursor-pointer hover:bg-slate-50 rounded transition-colors group">
                <div className="font-bold text-slate-800 group-hover:text-blue-500 text-lg">{stats.new}</div>
        ...
        <div className="space-y-3 pb-8">
          ...
          <button onClick={() => navigate("/game")} className="w-full bg-indigo-600 text-white font-bold py-5 px-6 rounded-2xl shadow-lg flex items-center justify-between group active:scale-95 transition-transform mb-2">
              <div className="flex items-center gap-4">
                ...
                <div className="text-left"><div className="text-xl">Flash Kart</div><div className="text-xs text-indigo-200 font-normal">{stats.target} kelime bekliyor</div></div>
            </div>
             <Play className="w-6 h-6 opacity-60 group-hover:translate-x-1 transition-transform"/>
          </button>

          {hardWordsCount > 0 && (
            <button onClick={() => navigate("/hard-words")} className="w-full bg-red-50 border-2 border-red-200 p-4 rounded-2xl flex items-center justify-between mb-4 shadow-sm active:scale-95 transition-all">
                <div className="flex items-center gap-3">
                    <div className="bg-red-100 p-2 rounded-lg text-red-600"><AlertTriangle className="w-6 h-6"/></div>
                    <div className="text-left"><div className="font-bold text-slate-800">Zorlandıklarım</div><div className="text-xs text-red-600 font-medium">Hata yapılanlar</div></div>
                </div>
                <div className="bg-red-500 text-white text-sm font-bold px-3 py-1 rounded-full shadow-sm">{hardWordsCount}</div>
            </button>
          )}
